...
def url_for_common_css(css_name: str) -> str:
    if not css_name.endswith(_CSS_EXTENSION):
        css_name += _CSS_EXTENSION
    return _url_for_static(_CSS_FILE_PATH.format(css_name), '')
...
def url_for_common_img(img_name: str) -> str:
    return _url_for_static(_IMG_FILE_PATH.format(img_name), '')
...
def url_for_common_js(js_name: str) -> str:
    if not js_name.endswith(_JS_EXTENSION):
        js_name += _JS_EXTENSION
    return _url_for_static(_JS_FILE_PATH.format(js_name), '')
